...
const std::string PlatformRiscV64::regName[PlatformRiscV64::maxRegNum] = {
    ...
    "sp",   // 类型：栈指针  作用： 指向当前栈顶 ,不需要栈保护，Callee
    ...
    "fs0",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs1",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    ...
    "fs2",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs3",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs4",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs5",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs6",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs7",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs8",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs9",  // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs10", // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
    "fs11", // 类型：保存(浮点) 作用： 被调用者保存寄存器 ,不需要栈保护，Callee
