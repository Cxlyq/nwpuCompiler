...
LocalVariable * Function::newArrayLocalVarValue(Type * type, std::string array_name, std::vector<int> dims, int32_t scope_level)
{
   ...
   ///todo int totalSize = arrayType->getSizeInBytes();  // 需要你自己实现
   ///TODO int offset = frameAllocator->allocate(totalSize);  // 栈帧分配器维护当前偏移

    ///TODO newArrayVar->setMemoryAddr(FP_REG, offset);  // 通常 FP_REG 是一个常量如 -1 表示 RBP
