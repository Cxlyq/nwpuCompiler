...
static std::string getIcmpPredicateString(IRInstOperator predicate)
{
    switch (predicate) {
        ...
        case IRInstOperator::IRINST_OP_GE_I:
            return "sge"; // Signed Greater Than or Equal To
        case IRInstOperator::IRINST_OP_LE_I:
            return "sle"; // Signed Less Than or Equal To
        case IRInstOperator::IRINST_OP_GNE_I:
            return "sgt"; // Signed Greater Than
        case IRInstOperator::IRINST_OP_LNE_I:
            return "slt"; // Signed Less Than
        default:
            return "invalid_icmp_predicate"; // Should not happen if used correctly
    }
}
...
/// @param _rhs 右操作数
///
IcmpInstruction::IcmpInstruction(Function * _func, IRInstOperator _predicate, Value * _lhs, Value * _rhs)
...
/// @param _rhs 右常数操作数
///
IcmpInstruction::IcmpInstruction(Function * _func, IRInstOperator _predicate, Value * _lhs, Constant * _rhsConst)
    // Call base class constructor: Instruction(function, opcode, result_type)
    // The opcode is the predicate itself. The result type of icmp is i1 (boolean).
...
void IcmpInstruction::toString(std::string & str)
{
    if (isRhsConst) {
        ...
        std::string predicate_str = getIcmpPredicateString(op); // 'op' is inherited from Instruction
        ...
        str = result_name + " = icmp " + predicate_str + " " + operand_type_str + " " + lhs_name + ", " + rhs_name;
    } else {
        // LLVM IR icmp format: %result = icmp predicate operand1_type operand1, operand2
        // Example: %t7 = icmp ne i32 %l3, 0
