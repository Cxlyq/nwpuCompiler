...
public:
    ...
    [[nodiscard]] bool isZeroValue() const override {
        for (const auto& val : values) {
            if (!val->isZeroValue()) return false;
        }
        return true;
    }
...
private:
    ...
    int32_t loadRegNo = -1;  // 寄存器编号


    //TODO // 正确性检查
    // 递归创建多维数组结构
    std::vector<Constant*> createMultiDimArray(const std::vector<Constant*>& elements, const std::vector<int>& dims, size_t dim) {
        if (dim == dims.size() - 1) {
        ...
        for (size_t i = 0; i < elementsPerDimension; ++i) {
            ...
            subArrays.push_back(new ConstArray(ArrayType::getArrayType(elements[0]->getType(), {dims[dim + 1]}), subElements, {dims[dim + 1]}));
        }
