...
bool IRGenerator::ir_const_declare(ast_node * node)
{
    ...
    if (id_node->node_type == ast_operator_type::AST_OP_ARRAY_ACCESS) {
        ...
        if (init_val_node) {
            std::vector<ast_node *>      init_list;
            std::vector<Instruction *> * insts = new std::vector<Instruction *>;
            if (!init_constarray_flattened(node->val, dims, init_val_node, *insts, init_list, var_type)) {
    ...
    switch (root->node_type) {
        ...
        case ast_operator_type::AST_OP_ARRAY_ACCESS: {
            ...
            for (size_t i = 1; i < root->sons.size(); ++i) {
                double val;
                if (!evaluateConstExpr(root->sons[i], &val)) // 递归处理，常量表达式的数组访问的下标也可能是表达式
                    return false;
        ...
        case ast_operator_type::AST_OP_NOT: {
            if (root->sons.size() != 1)
                return false;
            double operand;
            if (!evaluateConstExpr(root->sons[0], &operand)) // 递归调用，算出当前运算符之后式子的值
                return false;
        ...
        if (val_node) {
            if (!val_node->val) {
                ...
                if (val_type->isFloatType()) {
                    ...
                    std::cerr << "Error: Unsupported value type for array initialization." << std::endl;
                    return false;
                }
            }
...
bool IRGenerator::init_constarray_flattened(
    Value * arrayVar, const std::vector<int> & dims, ast_node * initNode, std::vector<Instruction *> & Insts,
    std::vector<ast_node *> & init_list, Type * val_type)
{
    bool isGlobalArray = false;
    ...
    InterCode & current_block_insts)
{
    ...
    if (op == ast_operator_type::AST_OP_AND || op == ast_operator_type::AST_OP_OR) {
        ...
        if (op == ast_operator_type::AST_OP_AND) {
            // 处理and短路情况，对于and，左值为true需要判断右值，左值为false则直接跳转至if-else的false标签
            ...
            // 递归调用处理右值短路，右侧true则跳转至if-else true，否则跳转至if-else false
            ...
            // 与处理and相似，不同为左值为true，直接跳转至if-else true，否则判断右值
            LabelInstruction * eval_right_label = new LabelInstruction(currentFunc);
    ...
    if (cond_type->isInt1Byte()) {
        ...
        Constant * zero_const = new ConstFloat(0.0f);
        if (!zero_const) {
